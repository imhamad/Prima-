...
$PATHWELL_STORAGE = array(
	...
	'required_plugins' => array_merge(
		...
		PATHWELL_THEME_FREE 
			? array(
					// Recommended (supported) plugins for the FREE (lite) version 'siteorigin-panels'			=> esc_html__('SiteOrigin Panels', 'pathwell'),
					) 
		...
		//-----------------------------------------------------
			: array(
					...
					'js_composer'				=> esc_html__('WPBakery Page Builder', 'pathwell'),
					'trx_updater'				=> esc_html__('ThemeREX Updater', 'pathwell')
					)
	),
	
	// Theme-specific URLs (will be escaped in place of the output)
	'theme_demo_url'	=> 'http://pathwell.axiomthemes.com',
	'theme_doc_url'		=> 'http://pathwell.axiomthemes.com/doc',
	'theme_download_url'=> 'https://themeforest.net/item/pathwell-senior-care-hospital-wordpress-theme/21975739',
...
if ( !function_exists( 'pathwell_importer_set_options' ) ) {
	...
	function pathwell_importer_set_options($options=array()) {
		if (is_array($options)) {
			...
			$options['files']['default']['domain_dev']  = esc_url(pathwell_get_protocol().'://pathwell.dv.axiomthemes.com');																	// Developers domain
			$options['files']['default']['domain_demo']= esc_url(pathwell_get_protocol().'://pathwell.axiomthemes.com');		// Demo-site domain
			...
			$options['banners'] = array(
				array(
					...
					'content' => wp_kses(__("Create your front page right in the WordPress Customizer. There's no need in WPBakery Page Builder, or any other builder. Simply enable/disable sections, fill them out with content, and customize to your liking.", 'pathwell'), 'pathwell_kses_content' ),
				...
				array(
					...
					'content' => wp_kses(__('Use Layouts Builder to create and customize header and footer styles for your website. With a flexible page builder interface and custom shortcodes, you can create as many header and footer layouts as you want with ease.', 'pathwell'), 'pathwell_kses_content' ),
				...
				array(
					...
					'content' => wp_kses(__('Need more details? Please check our full online documentation for detailed information on how to use Pathwell.', 'pathwell'), 'pathwell_kses_content' ),
				...
				array(
					...
					'content' => wp_kses(__('No time for reading documentation? Check out our video tutorials and learn how to customize Pathwell in detail.', 'pathwell'), 'pathwell_kses_content' ),
				...
				array(
					...
					'title' => esc_html__('Website Customization', 'pathwell'),
					'content' => wp_kses(__("Need a website fast? Order our custom service, and we'll build a website based on this theme for a very fair price. We can also implement additional functionality such as website translation, setting up WPML, and much more.", 'pathwell'), 'pathwell_kses_content' ),
...
if (!function_exists('pathwell_create_theme_options')) {
	...
	function pathwell_create_theme_options() {
		...
		$msg_override = wp_kses_data( __('<b>Attention!</b> Some of these options can be overridden in the following sections (Blog, Plugins settings, etc.) or in the settings of individual pages', 'pathwell') );
		...
		pathwell_storage_set('options', array(
			...
			'privacy_text' => array(
				...
				"std"   => wp_kses( __( 'I agree that my submitted data is being collected and stored.', 'pathwell'), 'pathwell_kses_content' ),
				"type"  => "text"
			),
			...
			'header_style' => array(
				...
				"desc" => wp_kses( __("Select custom header from Layouts Builder", 'pathwell'), 'pathwell_kses_content' ),
				"override" => array(
					'mode' => 'page,cpt_team,cpt_services,cpt_dishes,cpt_competitions,cpt_rounds,cpt_matches,cpt_cars,cpt_properties,cpt_courses,cpt_portfolio',
					'section' => esc_html__('Header', 'pathwell')
				),
			...
			'footer_style' => array(
				"title" => esc_html__('Select custom layout', 'pathwell'),
				"desc" => wp_kses( __("Select custom footer from Layouts Builder", 'pathwell'), 'pathwell_kses_content' ),
				"override" => array(
					'mode' => 'page,cpt_team,cpt_services,cpt_dishes,cpt_competitions,cpt_rounds,cpt_matches,cpt_cars,cpt_properties,cpt_courses,cpt_portfolio',
					'section' => esc_html__('Footer', 'pathwell')
				),
