...
if ( !function_exists( 'pathwell_options_add_scripts' ) ) {
	...
	function pathwell_options_add_scripts() {
		...
		if (is_object($screen) && $screen->id == 'appearance_page_theme_options') {
			...
			wp_enqueue_script( 'jq-colorpicker-colors', pathwell_get_file_url('js/colorpicker/colors.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jq-colorpicker', pathwell_get_file_url('js/colorpicker/jqColorPicker.js'), array('jquery'), null, true );
		...
		<div class="pathwell_options">
			...
			<form id="pathwell_options_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="pathwell_options_buttons">
					<input type="submit" value="<?php esc_attr_e('Save Options', 'pathwell'); ?>">
				</div>
			</form>
		</div>
		<?php
